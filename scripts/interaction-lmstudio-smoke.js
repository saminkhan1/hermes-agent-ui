...
const appArg = String(process.argv[2] || process.env.AGENT_UI_INSTALLED_APP || (fs.existsSync(localBundle) ? localBundle : installedBundle));
...
async function postJson(route, payload, opts = {}) {
  ...
  if (res.status < 200 || res.status >= 300) {
    fail(`POST ${route} failed: ${res.status} ${res.text}`);
  }
  return res.json;
}
...
function assertAssistiveAccessReady() {
  const res = runCommand('osascript', [
    '-e',
...
function clickAtRect(rect, label, { xRatio = 0.5, yRatio = 0.5 } = {}) {
  ...
  if (!res.ok) {
    fail(`${label} CGEvent click failed. macOS Accessibility permission may be required for real mouse event automation.\n${res.stderr || res.stdout}`);
  }
}
...
function assertNoErrorItems(conv, label) {
  const errors = (conv && Array.isArray(conv.items) ? conv.items : [])
...
async function waitModalVisible(label) {
  ...
  assertCondition(ui.modal.activeElement && ui.modal.activeElement.id === 'prompt', `${label}: prompt did not have focus`);
  return ui;
}
...
async function waitNewConversation(beforeIds, label) {
  return waitUntil(`new conversation: ${label}`, async () => {
...
async function waitAssistantSentinel(catId, sentinel, label, timeoutMs = 180000) {
  return waitUntil(`assistant sentinel ${sentinel}: ${label}`, async () => {
...
async function waitGatewayPostCount(catId, count, label) {
  return waitUntil(`gateway post count ${count}: ${label}`, async () => {
...
async function openConversationFromOverlay(catId, label) {
  const target = await waitUntil(`overlay row for ${catId}`, async () => {
  ...
  return waitUntil(`conversation window for ${catId}`, async () => {
    const ui = await uiTargets();
...
async function focusConversationFollowup(label) {
  ...
  return waitUntil(`${label}: follow-up input focused`, async () => {
    const next = await uiTargets();
    return next.conversation && next.conversation.activeElement && next.conversation.activeElement.id === 'followup-input'
      ? next
      : null;
  }, 5000);
}
...
async function runMenuShortcutPromptFollowupJourney() {
  ...
  assertCondition(String(initialPosts[0].conversationId || '') === catId, 'initial gateway conversation_id did not match the session');
  ...
  assertCondition(String(initial.gatewayConversationId || '') === catId, 'initial conversation recorded the wrong gateway id');
  ...
  pasteText(followup);
  await waitUntil('follow-up text pasted', async () => {
    const ui = await uiTargets();
  ...
  assertCondition(new Set(messageIds).size === 2, `initial and follow-up reused gateway message ids: ${messageIds.join(', ')}`);
  assertCondition(followupPosts[1].includeContext === false, 'follow-up unexpectedly included first-message context');
  assertCondition(followupPosts.every((post) => String(post.conversationId || '') === catId), 'follow-up changed gateway conversation_id');
  ...
  assertCondition(itemTexts(finalConversation, 'user').includes(followup), 'follow-up user text was not preserved exactly');
