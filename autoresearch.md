...
- 2026-05-02 stateful mascot pass: mapped mascot sprite rows to notification states (`running` run row, `review` jump row, `waiting` attack row, `failed` hurt row) so the top-priority notification changes the visible pet state like Codex. Scoped serial score improved to 88.725 while preserving checks.
